...
- **상태색 면 위 텍스트**: 원본은 일괄 `#fff`이지만, green/amber 위 흰색은 WCAG AA 미달(3.2~3.3:1)이라 색상별 on-토큰으로 교체 — `on-danger`는 흰색(4.8:1), `on-success`/`on-warning`은 진한색(5.5~5.9:1). 클로드디자인 쪽에 역반영 필요.
