...
실제 UI에 쓰는 **모든 전경/배경 조합**은 WCAG 2.2 AA를 통과해야 한다. 팔레트 원색끼리의 임의 조합이 아니라, 컴포넌트와 화면에서 실제로 맞닿는 시맨틱 토큰 조합을 판정한다.

- 일반 텍스트(굵기·크기와 무관하게 기본 판정): **4.5:1 이상**. 작은 메타·자리 표시자·상태 문구도 예외가 아니다.
- 큰 텍스트(24px 이상 또는 굵은 18.66px 이상): **3:1 이상**. 크기에 기대기보다 4.5:1을 기본으로 잡는다.
- 입력·버튼·체크·스위치의 식별 경계, 포커스 표시, 의미가 필요한 아이콘·차트 표식: 인접 면과 **3:1 이상**.
- 비활성 컨트롤은 최소 대비 예외지만, 활성 상태와 같은 토큰을 불투명도만 낮춰 만들지 않는다.
- 상태를 색만으로 전하지 않는다. 색 대비를 통과해도 문장·아이콘 모양·범례·직접 라벨 중 하나를 함께 둔다.
- 반투명 색은 최종 합성된 색으로 계산하고, 라이트·다크·호버·포커스·솔리드·연한 면을 각각 검증한다.

슬러 디자인은 `npm run check:contrast`를 빌드 전 실행해 정의된 조합을 검사한다. 토큰이나 배경을 바꾸면서 검사 목록을 줄여 통과시키는 것은 금지하며, 새 조합을 만들면 검사 쌍을 함께 추가한다.

— WCAG 1.4.3 Contrast (Minimum) · 1.4.11 Non-text Contrast · 2.4.11 Focus Appearance

### 보류 (다음 후보)

터치 타깃 24px(2.5.8 / KWCAG 6.1.3), 문자 단축키(2.1.4 / KWCAG 6.1.4), 접근 가능한 인증(3.3.8 / KWCAG 7.3.3). `prefers-reduced-motion`은 global.css에 이미 동봉되어 있다.
