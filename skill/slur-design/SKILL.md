...
| `modal_dialog` — `<dialog>` 1순위(`[open]` 정본), `div` 대안은 `data-state` | `components/modal.css` |
...
| `toast_message`(`role="status|alert"`, `data-state="show|close"`, + `m_top` `m_success` `m_danger`) | `components/toast.css` |
| `empty_state`(+ `m_error` `m_compact`), `skeleton`(+ `m_title` `m_circle` `m_rect` `m_inline`), `spinner`(+ `m_large`) | `components/state.css` |
...
4상태 슬롯(`i_loading`/`i_empty`/`i_error`/`i_body`)의 **노출 토글은 `global.css`**(slur-guidelines)가 하고, 이 시스템은 그 안에 넣는 `empty_state`·`skeleton`·`spinner`의 룩만 준다.
