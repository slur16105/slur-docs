---
name: slur-design
description: SLUR Design System — 시각 어휘(디자인 토큰과 컴포넌트 스타일). SLUR 룩앤필로 UI를 만들 때 이 스킬을 사용한다. "슬러 디자인", "SLUR 디자인으로", "우리 디자인시스템으로", "슬러 룩으로 만들어줘"라는 요청, 또는 SLUR 토큰(색·타이포·간격)이나 기성 컴포넌트(Button, Input, Badge 등)가 필요할 때 참고한다. 호칭 구분 — "슬러 디자인"은 이 스킬과 slur-ux 스킬을 **함께** 쓰라는 뜻이다(어휘만 따로 쓸 수 없고 문법이 있어야 성립). 반대로 "슬러 시스템"(단독)이라고 하면 slur-ux 만 적용하고 기존 디자인은 건드리지 않는다.
---
...
- **모바일 터치 타깃은 48이 기본**이다 — 최소 권장(Material 48·Apple 44) 충족. 아이콘만 있는 단독 버튼은 보이는 크기 = 탭 영역이므로 48×48로 잡는다. 햄버거(메뉴) 버튼은 `.btn_menu` 블록(48×48 고정, 테두리·배경 없이 아이콘만)을 쓴다.
