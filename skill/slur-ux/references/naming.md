...
- 내부 요소는 **역할**로 이름 짓는다 — HTML 태그 타입(button, div, img…)이 아니라. 예: 닫기 버튼은 `i_close`(역할)이지 `i_btn`(타입)이 아니다. 검색 영역은 `l_search`이지 `l_input`이 아니다. 업그레이드 CTA 버튼은 `l_pro`이지 `l_btn`이 아니다.
