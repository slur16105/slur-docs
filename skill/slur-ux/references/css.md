...
### 기존 프로젝트 이관 시에는 깊이를 줄이지 않는다

2단계 권장은 **새로 작성하는 코드**에 적용한다. 이미 돌아가는 프로젝트에 SLUR을 도입할 때는 **기존 선택자의 깊이를 그대로 둔다.**

이름을 바꾸는 것(`#header`→`.layout_header`)은 특이도가 유지되는 무손실 변환이지만, 깊이를 줄이는 것(`.page_a .p_b .p_c`→`.page_a .p_c`)은 **특이도가 함께 떨어져 다른 규칙에 밀릴 수 있다.** 밀리는 지점은 화면을 전부 눈으로 확인하기 전에는 드러나지 않고, 이관 작업의 "표기법만 바꾸고 화면은 그대로"라는 전제를 깨뜨린다.

```css
/* 이관 시 O — 이름만 바꾸고 깊이는 유지 */
.page_column_view .p_qna .p_qna_list li .p_it { }

/* 이관 시 X — 깊이를 줄이면 특이도가 낮아져 밀릴 수 있다 */
.page_column_view .p_it { }
```

깊이 정리는 이관이 끝나고 CSS를 다시 설계할 때(컴포넌트 분리·파일 분할 단계) 화면 확인과 함께 한다.
