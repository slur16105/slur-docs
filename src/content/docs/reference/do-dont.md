...
위 사례들을 관통하는 지양 패턴과 각각의 정본 규칙 문서입니다.

- **역할이 모호한 클래스** — 하나의 클래스가 구조·스타일·상태를 동시에 담당하면 수정 범위를 예측할 수 없습니다. → [역할 분리 원칙](/core/separation-of-concerns/)
- **과도한 중첩 선택자** — 상위 구조에 의존한 깊은 선택자는 구조 변경 시 연쇄적으로 깨집니다. → [중첩 구조 규칙](/css/nested-structure/)
- **시각적 표현 중심 네이밍** — 표현이 바뀔 때마다 구조 수정으로 이어집니다. 네이밍은 [접두사 기준표](/reference/prefix-table/), 값은 [토큰](/css/tokens/)으로 관리합니다.
- **구조를 대체하는 JavaScript** — UI 구조를 임의로 생성·삭제하면 구조를 추적하기 어렵습니다. → [상태 기반 UI](/js/state-driven-ui/)
- **임의 값 사용** — 기준 없는 값이 늘어날수록 일관성이 저하됩니다. → [토큰](/css/tokens/)
