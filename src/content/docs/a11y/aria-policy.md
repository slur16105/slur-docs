...
## 상태 변화를 보조 기술에 알리기 (라이브 영역)

시각적으로만 표현되는 상태 변화는
라이브 영역으로 보조 기술에 전달합니다.

- 토스트·상태 메시지의 기본은 `role="status"`입니다. 끼어들어야 하는 실패·세션 만료만 `role="alert"`를 씁니다.
- 라이브 영역 컨테이너는 미리 DOM에 있어야 읽힙니다. "빈/로딩/에러/정상 4상태를 마크업에 미리 둔다"는 원칙과 같은 이유입니다. 4상태 안내문은 항상 렌더되는 `i_status`(`role="status"`) 상자 안에 두고 안내문만 전환합니다 — 상자 자체가 숨었다 나타나면 읽히지 않습니다([화면 상태 설계](/ux/screen-states/)).
- 알림은 포커스를 옮기지 않습니다.
- `role="alert"` 메시지는 자동으로 사라지게 하지 않습니다. `status` 토스트는 자동 소멸을 허용하되, 같은 정보를 다른 곳에서 다시 확인할 수 있어야 합니다.
...
툴팁은 호버와 포커스 모두에서 뜨고,
Esc로 포커스 이동 없이 닫히며,
마우스를 툴팁 위로 옮겨도 유지되어야 합니다.

연결은 `aria-describedby`(설명) 또는 `aria-labelledby`(라벨)로 합니다.
툴팁 안에 버튼·링크를 넣지 않습니다 —
그것은 툴팁이 아니라 팝오버/다이얼로그입니다.
...
셀렉트는 네이티브 `<select>`를 먼저 씁니다.
커스텀 콤보박스·날짜선택기·메뉴바가 꼭 필요하면
검증된 헤드리스 라이브러리에 동작(키보드·ARIA)을 맡기고,
SLUR는 구조·네이밍·`data-state`만 얹습니다.

[외부 라이브러리 연동](/js/external-libraries/)의 원칙 —
라이브러리는 상태 트리거일 뿐 — 과 같은 선상입니다.
헤드리스 라이브러리 다수가 상태를 `data-state` 속성으로 노출하므로
SLUR 네이밍과 그대로 호환됩니다.
