...
- 내부 요소는 특정 블록 안에서만 쓰이므로, 수정 시 영향 범위가 그 블록으로 **국소화**됩니다 — 무엇이 깨질지 고민할 필요가 적습니다.
- 반대로 컴포넌트로 분리하면 **어디에서 쓰이는지 추적**해야 하므로 함부로 고치지 못하고, 사용처를 전부 확인하는 **관리 비용**이 계속 발생합니다.

그래서 "나중에 재사용될지 몰라서" 미리 컴포넌트로 빼지 않습니다.
재사용이 실증되기 전까지는 종속이 더 싸고 안전합니다.
