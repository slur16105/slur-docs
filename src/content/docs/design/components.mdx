...
<section class="p_programs" data-state="loading" aria-label="프로그램 목록">
  <div class="i_status" role="status">            <!-- 항상 렌더 — 상태 전환이 보조 기술에 읽힌다 -->
    <div class="i_loading"><span class="skeleton"></span><span class="a11y_hidden">불러오는 중…</span></div>
...
`data-state` 값만 `loading` → `empty` / `error` / `success`로 바꾸면 `global.css`의 스위치가 슬롯을 고릅니다. 룩(스켈레톤·빈 상태 패널·스피너)은 `state.css`가 줍니다 — [화면 상태 설계](/ux/screen-states/).
