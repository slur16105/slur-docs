...
## 요소는 역할로 이름 짓는다 (태그 타입이 아니라)

내부 요소의 이름은 **그 요소가 무슨 일을 하는가(역할)** 를 기준으로 정합니다.
`button`·`div`·`img` 같은 **HTML 태그 타입**을 이름으로 삼지 않습니다.

- 닫기 버튼 → `i_close` (O) / `i_btn` (X)
- 검색 영역 → `l_search` (O) / `l_input` (X)
- 업그레이드 CTA 버튼 → `l_pro` (O) / `l_btn` (X)

같은 태그라도 역할이 다르면 다른 이름을 가지며, 역할이 같으면 같은 이름을 씁니다.
타입 기반 이름(`l_btn`)은 같은 태그가 여러 개일 때 충돌하고, 구조적 의미를 드러내지 못합니다.
