...
- **브랜드와 강조색 분리** — SLUR 로고는 원본 블랙을 유지하고, 블루는 제목·구분선·정보 강조에만 사용한다. 테마색으로 로고를 다시 칠하지 않는다.
- **블루 권장안 확정** — 넓은 면은 흰색·연회색으로 두고 블루 계열로 정보 구조를 만든다. 빨간색은 실제 위험을 알리는 작은 표시에만 제한한다.
- **요약본 일반어 우선** — 비개발자가 먼저 보는 요약본은 기술 용어 대신 일상적인 표현을 사용하고, 정확한 측정값·기술 설명·확인 근거는 상세 보고서에서 제공한다.
- **PDF 생성·검수 절차 기록** — 벡터 PDF를 직접 생성한 뒤 페이지 이미지로 렌더링해 글자 잘림, 겹침, 로고 색, 경고색 비중을 확인한다.

---

## v1.12.0 — 2026.08

### WCAG AA 색 대비를 배포 기준으로 확정

- **예외 없는 실제 조합 기준** — 일반 텍스트는 4.5:1, 입력·버튼·체크·스위치 경계와 포커스 표시·차트 표식은 3:1 이상으로 확정했다. 작은 메타·자리 표시자·상태 문구도 일반 텍스트 기준을 적용하고 라이트·다크·호버·솔리드·연한 면의 최종 합성색을 각각 판정한다.
- **토큰 재조정** — 라이트 muted, 다크 secondary/muted, 기본·강한 보더, 브랜드 기본색과 다크 브랜드 텍스트를 조정했다. 반투명 포커스 링은 불투명 `border-focus` 색으로 바꿨다. 상태 전경과 솔리드 면을 `--color-*-solid`로 분리하고, 토스트 반전 면에는 `--color-*-on-inverse`를 추가해 성공 솔리드의 기존 3.3:1 예외를 제거했다.
- **컴포넌트·차트 보정** — 솔리드 배지·위험 버튼·온보딩 완료 아이콘은 solid 토큰을 사용하고, 연한 브랜드 면의 아이콘은 `text-brand`를 사용한다. 입력 자리 표시자를 명시하고, 스위치의 꺼짐/켜짐 노브 색을 분리했다. 차트 다섯 슬롯은 두 테마의 카드 면에서 모두 3:1 이상이며 도넛 조각 사이에 카드 면 구분선을 넣었다.
- **배포 차단 자동 검사** — `scripts/check-color-contrast.mjs`가 정의된 140개 조합을 검사한다. `prebuild`에 연결해 실패하면 빌드·배포를 중단한다. 현재 최소값은 일반 텍스트 4.53:1, UI·포커스·차트 3.07:1이다.

---

## v1.11.5 — 2026.08

### 페이지 콘텐츠 기본 좌우 거터 16px

- 기존에는 레이아웃 구조 간격을 px로 직접 쓴다는 단위 규칙만 있고 페이지 좌우 거터의 기준값과 근거가 없었다. 앱 셸의 헤더·본문 콘텐츠 좌우 거터를 **16px**로 통일했다. 4px 그리드 정합성, 좁은 화면의 유효 폭, 화면 간 시작선 일관성이 근거다. 카드 내부 패딩·섹션 간격은 적용 대상이 아니다. 디자인 토큰·패턴 문서, 레시피, 스킬 체크리스트에도 같은 기준을 기록했다.
- Cloudflare Pages가 Astro 7을 지원하지 않는 Node 20으로 빌드하지 않도록 `.node-version`을 22.23.0으로 고정했다. 로컬·GitHub Actions·Pages가 같은 Node 22 계열을 사용한다.
...
구현 규칙을 실제 사용 가능 여부와 연결하는 공개 검증 기준을 추가했습니다.

- 자동 측정·실제 브라우저 검증·사람 판단의 세 층을 분리
- 모바일·태블릿·데스크톱·320px 공통 화면 기준 명문화
- 키보드, 실패·복구, 동적 콘텐츠의 실제 흐름 검증 절차 추가
- 총점으로 상쇄하지 않는 출시 차단 조건 정리
- 배포·인계 전 완료 검증 체크리스트 신설
- 고객별 점수·증거·실행 데이터는 공개 UX/UI 문서와 분리
